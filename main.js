...
HERO_DOTS.forEach((dot,index) => dot.addEventListener('click', event => {
    ...
    HERO_IMAGE.src = `assets/hero-${index+1}.png`;
...
// Для чего
const FEATURES_DOTS = document.querySelectorAll('.features__dot');
const FEATURES_CAPTION = document.querySelector('.features__item-caption');

const FEATURES_CAPTIONS = [
...
FEATURES_DOTS.forEach((dot,index) => dot.addEventListener('click', event => {
    FEATURES_DOTS.forEach(el => {
        el.classList.remove('active');
    })
    dot.classList.add('active');

    FEATURES_CAPTION.textContent = FEATURES_CAPTIONS[index];
}))